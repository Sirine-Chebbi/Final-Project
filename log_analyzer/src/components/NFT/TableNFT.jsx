...
const TableNFT = ({
  ...
  const { minLimit, maxLimit } = useMemo(() => {
    ...
    const limits = filteredResults.reduce(
      (acc, result) => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, "NFT_results.xlsx");
  };
          ...
          <tbody className="divide-y divide-cyan-400 bg-gray-900">
            {filteredResults.length > 0 ? (
              ...
              <tr>
                <td
                  colSpan="10"
                  className="px-6 py-4 text-center text-lg text-cyan-400"
                >
                  Aucune donnée disponible pour les filtres :
                  <br />
                  Mesure: {mesure || "Aucun"}, Antenne: {antenne || "Aucun"},
                  Bande: {bande || "Aucun"}
                </td>
              </tr>
            )}
