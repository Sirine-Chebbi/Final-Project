...
const Filters = ({ setSelectedFrequency, setVisibility, setSelectedAntenne, setSelectedCaisson}) => {
    ...
    return (
        <>
            <input type="text" id="inputc" placeholder="Caisson" onChange={(e) => setCaisson(e.target.value)} className="border-3 border-red-400 w-50 p-5 rounded-2xl text-xl font-medium text-red-400 h-15 mb-5 mt-10 outline-none" />
            <button onClick={() =>  handleClickCaisson(caisson)} className="text-black border-3 hover:border-red-400 hover:text-red-400  bg-red-400 hover:bg-gray-950 focus:outline-none h-15 font-medium rounded-2xl w-35 ml-5 text-xl px-4 py-2 cursor-pointer">
                    Enregistrer
            </button>
