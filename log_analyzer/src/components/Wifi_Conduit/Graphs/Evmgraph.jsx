...
const Evmgraph = ({filteredResults, selectedCaisson}) => {

    const calculateStats = (data) => {
        
      if (!data || data.length === 0) return null;
    
      const powerValues = data.map((item) => parseFloat(item.evm)).filter((p) => !isNaN(p));
          ...
          {filteredResults.length > 0 ? (
            <div className="p-6 bg-gray-800 rounded-lg mt-30 hover:scale-102 duration-200 hover:shadow-cyan-400 shadow-2xl mb-20">
              <h2 className="text-2xl text-cyan-400 mb-4">
                Evm - {filteredResults[0]?.frequence}Hz - Antenne {filteredResults[0]?.ant} || Caisson: {selectedCaisson}
              </h2>
