...
const Deltagraph = ({ Results, selectedCaisson, setShowAi, setStatData, setType }, ref) => {
  ...
  const calculateStats = (data) => {
    ...
    const powerValues = data
      .map((item) => parseFloat(item.delta))
      .filter((p) => !isNaN(p));
          ...
          <div className="p-6 bg-gray-800 rounded-lg mt-30 hover:scale-102 duration-200 hover:shadow-cyan-400 shadow-2xl mb-20">
            <div className="flex justify-between mb-4">
              ...
              <div className="flex gap-5">
                <div onClick={() => setShowAi(true) | setType("rx") | setStatData(statData)} className={`hover:bg-gray-700 duration-200 border-2 border-yellow-400 rounded-xl p-1 text-yellow-400 px-5 h-10 place-items-center cursor-pointer flex ${datauser.role != "1" ? 'hidden' : ''}`}>
